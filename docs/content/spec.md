...
Global options are `--db`, `--json`, and `--query-budget`. The database path resolves in this order: explicit `--db`, then `FGRAPH_DB`, then an implicit local selection. An explicitly selected empty path, including a present-but-empty `FGRAPH_DB`, MUST fail with `FormatError`. For an implicit selection, a legacy-only `fgraph.db` MUST remain selected for v1 compatibility; otherwise the default is `facts.fgraph`. When both paths exist, the command MUST select `facts.fgraph` only after verifying that it is an initialized fgraph database; an empty or unrelated new default MUST fail with `FormatError` before mutation. Explicitly selecting either path overrides this transition. Help, version, and invalid usage MUST be handled without inspecting either database path. `add`, declarations, shapes, `schema-apply`, and undo accept `--operation-id`/`--if-basis-tx`; irreversible `excise` requires both. Machine output is canonical JSON; protocol streams are NDJSON. Exit status is 0 success, 1 typed runtime error, and 2 usage error. Other environment configuration is `FGRAPH_CLOCK`, `FGRAPH_QUERY_BUDGET`, and the test/reproducibility-only `FGRAPH_EVENT_SEED`.
